#!/usr/bin/env luajit
-- Simple eggdrop log parser and db inserter
local logging = require'logging.console'
local log = logging()
local dbi = require 'DBI'
local dbh, err = DBI.Connect('PostgreSQL', 'irc', os.getenv('DBUSER'), os.getenv('DBPASS'), '127.0.0.1', '5432')
if err then
    log:error(err)
    return
end

local function ltrim(s, pat)
  return (s:gsub("^"..pat, ""))
end
...
if arg[1] == nil  or arg[2] == nil then
    print ('Usage: eggdroplogurlhandler.lua channelname logfile')
    os.exit(1)
end
