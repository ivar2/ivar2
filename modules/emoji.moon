...
-- Cache the parsed json str
emojitrans = util.json.decode(emojitranslatejson)

indexOf = (t, object) -> -- all hail JavaScript
  if "table" == type(t)
    for i = 1, #t
      if object == t[i] then
        return i
...
emojify = (source, dest, arg) =>
  -- translate english to emojis, thanks to notwaldorf/emoji-translate
  say arg\gsub '%w+', (origword) ->
    word = origword\lower!

    -- Maybe this is a plural word but the word is the singular?
    -- Don't do it for two letter words since "as" would become "a" etc.
    maybeSingular = ''
    if #word > 2 and word\sub(1) == 's'
      maybeSingular = word\wub(0, -2)
    ...
    maybePlural = if #word == 1 then '' else word .. 's'

    -- Go through all the things and find the first one that matches.
    for emoji, vals in pairs emojitrans
      words = vals.keywords
      if emoji == word or emoji == maybeSingular or emoji == maybePlural or (words and indexOf(words, word) >= 0) or (words and indexOf(words, maybeSingular) >= 0) or (words and indexOf(words, maybePlural) >= 0)
        if vals.char != util.json.null
          return vals.char
...
PRIVMSG:
  ...
  '^%pemojify (.+)$': emojify
  '^%pemojitrans (.+)$': emojify
  '^%pemojiall$': (source, dest, arg) =>
    out = {}
    for cat, str in pairs emojis
      table.insert out, str
    say table.concat out
  '^%pemojicats$': (source, dest, arg) =>
    out = {}
    for cat, str in pairs emojis
      table.insert out, cat
    say table.concat(out, ', ')
  '^%pemojicat (.+)$': (source, dest, cat) =>
    say emojis[cat]
