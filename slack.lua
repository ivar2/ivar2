...
local slack_api = require'core/slack_api'
...
function ivar2:Privmsg(destination, format, ...)
	...
	return slack_api:Privmsg(destination, message)
end
...
function ivar2:Connect(config)
	...
	queue:wrap(function()
	  slack_api:Connect(config.password)
	end)
	slack_api:RegisterHandler('message', function(m)
	  local source = slack_api:get_user(m.user)
	  local destination = slack_api:get_channel(m.channel)
	  if not destination then
		destination = m.channel
	  else
		destination = '#'..destination
	  end

	  local argument = m.text
